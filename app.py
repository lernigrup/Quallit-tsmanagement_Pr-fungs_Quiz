...
import io
APP_ACTIVE = True  
APP_VERSION = "2026-ALLE-PROBEKLAUSUREN-V2"
...
QUIZZES = {
    "Altklausuren 2025": "Altklausuren 2025.json",
    "Probeklausur 1 2025": "Probeklausur 1 2025.json",
    "Probeklausur 2 2025": "Probeklausur 2 2025.json",
    ...
    "Alle Probeklausuren 2026": "Alle Probeklausuren 2026.json",
}

# Alternative Dateinamen, falls du die Datei nicht umbenennst.
# Die App probiert zuerst den Namen aus QUIZZES und danach diese Alternativen.
QUIZ_FILE_ALIASES = {
    "Alle Probeklausuren 2026": [
        "questions_m10fs_sose2026_alle_probeklausurfragen_flat.json",
        "questions_m10fs_sose2026_probeklausuren_1bis3.json",
    ],
}
...
def get_selected_quiz_label() -> str:
    ...
    if label in QUIZZES:
        return label
    return next(iter(QUIZZES))

def get_questions_file() -> Path:
    label = get_selected_quiz_label()
    fname = QUIZZES.get(label)
    path = BASE_DIR / fname

    # Falls die Datei nicht exakt so heißt, probiere hinterlegte Alternativnamen.
    if not path.exists():
        for alt_name in QUIZ_FILE_ALIASES.get(label, []):
...
if st.session_state.get("quiz_label") is None:
    ...
    st.caption("Hinweis: Die Quizze werden aus den JSON-Dateien im Repository geladen. Für 2025 und 2026 sind eigene Probeklausuren hinterlegt.")
    st.stop()
...
if not questions:
    st.error(f"Keine Fragen gefunden. Erwartete Datei: {get_questions_file().name}. Prüfe den Dateinamen und ob die JSON-Datei Inhalt hat.")
    st.stop()
